...
Case studies are added one at a time, each requiring explicit approval before it is built. Of the 12 case studies registered so far, CS1, CS2, CS3, CS4, CS5, CS9, CS10, and CS11 are complete at all 8 stages. CS6's retrofit is in progress: stages 1, 3, and 4 already existed and now run cleanly after fixing a DeadKernelError and replacing a scientifically bogus synthetic-data fallback (Hippocorpus is gated with no programmatic access) with a real, mechanism-driven text-decay generator; stages 2, 5, 6, 7, and 8 are still to be built. CS7-8 will be migrated to the full 8-stage pipeline next, one at a time and in numeric order, along with a narrative-quality pass bringing their history and science sections up to CS1's standard of named historical figures, dated external citations, and a built cause-and-effect story rather than a reference table. CS9-11 already have all 8 stages structurally but are being migrated from their Hugging Face LLM backend to NVIDIA NIM for consistency with CS1-2.

CS12 adds speech and audio to the project's field list, an approved extension of scope rather than a substitution for any field already queued. It is newly registered and currently a scaffold. Its notebook carries the full 18-section house structure, the history and science sections, and the Rediscovery Standard, with every measured result marked `TODO(numbers)`. Stage 1 covers a Decision Tree, a Random Forest, and XGBoost, and stage 2 covers a PyTorch feedforward network, each run under two MFCC feature representations, a mean-pooled 40-feature vector and the full flattened 1,280-feature spectrogram, so that the accuracy gap between them isolates the contribution of the time axis. Stage 3 substitutes wav2vec 2.0 for TabPFN, which does not fit an audio domain, and stage 4 uses permutation importance, which folds back onto the 40 by 32 grid in a way SHAP and LIME do not. The case study's evaluation is still being computed, so no accuracy figure appears anywhere in the notebook or this README yet.
