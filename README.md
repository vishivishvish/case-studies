...
A series of AI/ML case studies, designed for citizen AI engineers and data scientists who need to work across various domains and gain consultant-level expertise in each one.
...
| 12 | [Spoken Word Recognition from Spectrograms](012%20-%20Spoken%20Word%20Recognition%20from%20Spectrograms/spoken_word_recognition.ipynb) | Speech recognition / audio machine learning | One of twenty spoken words (the digits zero through nine and ten command words) from MFCC spectrograms (Google Speech Commands v0.02, roughly 77,000 clips from roughly 2,580 speakers, split speaker-disjoint) | Candidate: the Haskins Laboratories formant-transition finding (Delattre, Liberman, and Cooper, 1955), that word identity is carried by spectral change over time, which a time-averaged spectrum discards. TODO(numbers): Section 15 has not run yet, and the law gets dropped if the test fails, as Granovetter was in CS4 | Scaffold only, no stage executed yet |
...
Case studies are added one at a time, each requiring explicit approval before it is built. Of the 12 case studies registered so far, CS1, CS2, CS3, CS4, CS5, CS9, CS10, and CS11 are complete at all 8 stages. CS6's retrofit is in progress: stages 1, 3, and 4 already existed and now run cleanly after fixing a DeadKernelError and replacing a scientifically bogus synthetic-data fallback (Hippocorpus is gated with no programmatic access) with a real, mechanism-driven text-decay generator; stages 2, 5, 6, 7, and 8 are still to be built. CS7-8 will follow to the full 8-stage pipeline next, one at a time and in numeric order, along with a narrative-quality pass bringing their history and science sections up to CS1's standard of named historical figures, dated external citations, and a built cause-and-effect story rather than a reference table. CS9-11 already have all 8 stages structurally but are being migrated from their Hugging Face LLM backend to NVIDIA NIM for consistency with CS1-2.

CS12 is newly registered and currently a scaffold. Its notebook carries the full 18-section house structure, the history and science sections, and the Rediscovery Standard, with every measured result marked `TODO(numbers)` and stages 3, 5, 6, 7, and 8 marked `TODO(stage)`. Stage 1 covers a Decision Tree, a Random Forest, and XGBoost, and stage 2 covers a PyTorch feedforward network, each run under two MFCC feature representations, a mean-pooled 40-feature vector and the full flattened 1,280-feature spectrogram, so that the accuracy gap between them isolates the contribution of the time axis. Stage 3 substitutes wav2vec 2.0 for TabPFN, which does not fit an audio domain. Two items need an explicit decision before the build proceeds: speech and audio is not currently on the project's confirmed field list, so adding it is a scope change, and the case study's evaluation is still being computed, so no accuracy figure appears anywhere in the notebook or this README yet.
